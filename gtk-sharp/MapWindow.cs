...
public partial class MapWindow
{
    ...
    public Dictionary<int, LevelAttributes> AllLevelAttributes { get; set; } = [];
    ...
    SortedList<short, bool> paintIndexes = [];
    List<Gdk.Color> paintColors = [];
    ...
    {
        button.IconWidget = new Gtk.Image(null, "weland.images." + resource);
    ...
    public MapWindow(string title)
    {
        ...
        var gxml = new Glade.XML(null, "weland.glade.mapwindow.glade", "window1", null);
        ...
        var width = Weland.Settings.GetSetting("MapWindow/Width", 800);
        var height = Weland.Settings.GetSetting("MapWindow/Height", 600);
        window1.Resize(width, height);

        var top = Weland.Settings.GetSetting("MapWindow/Top", 0);
        var left = Weland.Settings.GetSetting("MapWindow/Left", 0);
    ...
    void UpdateStatusBar()
    {
        ...
        {
            var line = Level.Lines[selection.Line];
        ...
        {
            var obj = Level.Objects[selection.Object];
        ...
        {
            var note = Level.Annotations[selection.Annotation];
        ...
        {
            var p0 = Level.Endpoints[Level.TemporaryLineStartIndex];
            var p1 = Level.TemporaryLineEnd;
            var X = p1.X - p0.X;
            var Y = p1.Y - p0.Y;
    ...
    void AdjustScrollRange()
    {
        ...
        var scale = drawingArea.Transform.Scale;
        var VUpper = short.MaxValue - (height / scale);
        ...
        var HUpper = short.MaxValue - (width / scale);
    ...
    [GLib.ConnectBefore()]
    protected void OnWindowConfigure(object obj, ConfigureEventArgs args)
    {
        int X, Y;
    ...
    void RedrawDirty()
    {
        ...
        if (editor.Dirty)
        {
            ...
            {
                var X1 = (int)drawingArea.Transform.ToScreenX(editor.RedrawLeft) - dirtySlop;
                var Y1 = (int)drawingArea.Transform.ToScreenY(editor.RedrawTop) - dirtySlop;
                var X2 = (int)drawingArea.Transform.ToScreenX(editor.RedrawRight) + dirtySlop;
                var Y2 = (int)drawingArea.Transform.ToScreenY(editor.RedrawBottom) + dirtySlop;
    ...
    {
        var behavior = (ScrollWheelBehavior)Weland.Settings.GetSetting("ScrollWheelBehavior", (int)ScrollWheelBehavior.Zoom);
        if (behavior == ScrollWheelBehavior.Zoom)
        {
            var X = drawingArea.Transform.ToMapX(args.Event.X);
            var Y = drawingArea.Transform.ToMapY(args.Event.Y);
    ...
    {
        var X = drawingArea.Transform.ToScreenX(Xt);
        var Y = drawingArea.Transform.ToScreenY(Yt);
        var originalScale = drawingArea.Transform.Scale;
        var scale = originalScale * factor;
        if (scale > 4)
        {
            scale = 4;
        }
        ...
        editor.Scale = scale;
        var xNew = (int)Math.Round((X / originalScale) - (X / scale) + drawingArea.Transform.XOffset);
        var yNew = (int)Math.Round((Y / originalScale) - (Y / scale) + drawingArea.Transform.YOffset);
    ...
    {
        var X = drawingArea.Transform.ToMapX(drawingArea.Allocation.Width / 2);
        var Y = drawingArea.Transform.ToMapY(drawingArea.Allocation.Height / 2);
        ZoomIn(X, Y);
    }
    ...
    void EditAnnotation()
    {
        var annotation = Level.Annotations[selection.Annotation];
        var d = new EntryDialog("Annotation Text", window1)
        {
    ...
    {
        var ev = args.Event;
        var X = drawingArea.Transform.ToMapX(ev.X);
        var Y = drawingArea.Transform.ToMapY(ev.Y);
        ...
        if (ev.Type == EventType.ButtonPress)
        {
            ...
            {
                var modifiers = Modifiers(ev.State);
                if (ev.Button == 3)
                {
        ...
        else if (ev.Type == EventType.TwoButtonPress)
        {
            if (selection.Line != -1)
            {
                var d = new LineParametersDialog(window1, Level, Level.Lines[selection.Line]);
                d.Run();
                editor.Changed = true;
                Redraw();
            }
            ...
            {
                var d = new PointParametersDialog(window1, Level, selection.Point);
                d.Run();
                editor.Changed = true;
                Redraw();
            }
    ...
    internal void OnButtonReleased(object obj, ButtonReleaseEventArgs args)
    {
        ...
        {
            var modifiers = Modifiers(args.Event.State);
        ...
        if (editor.Tool == Tool.FloorHeight || editor.Tool == Tool.CeilingHeight || editor.Tool == Tool.PolygonType || editor.Tool == Tool.FloorLight || editor.Tool == Tool.CeilingLight || editor.Tool == Tool.MediaLight || editor.Tool == Tool.Media || editor.Tool == Tool.AmbientSound || editor.Tool == Tool.RandomSound)
        {
            ...
            var i = paintIndexes.IndexOfKey(editor.PaintIndex);
            if (i != -1)
            {
                var crb = (ColorRadioButton)paletteButtonbox.Children[i];
                if (!crb.Active)
                {
                    crb.Active = true;
                }
        ...
        if (selection.Line == -1)
        {
            gridResetButton.Label = "Reset";
        }
        else
        {
            gridResetButton.Label = "Set All";
        }
    ...
    [GLib.ConnectBefore()]
    internal void OnSpecialKeyPressed(object obj, KeyPressEventArgs args)
    {
        args.RetVal = true;
        if (PlatformDetection.IsMac)
        {
    ...
    [GLib.ConnectBefore()]
    internal void OnSpecialKeyReleased(object obj, KeyReleaseEventArgs args)
    {
    ...
    public bool CheckSave()
    {
        if (editor.Changed)
        {
            var dialog = new MessageDialog(window1, DialogFlags.DestroyWithParent, MessageType.Warning, ButtonsType.None, "Do you wish to save changes?");
    ...
    void BuildLevelMenu()
    {
        ...
        if (mapfile.Directory.Count > 1)
        {
            ...
            foreach (var kvp in mapfile.Directory)
            {
                ...
                {
                    var item = new MenuItem(mapfile.Overlays[kvp.Value.Index].LevelName);
                    var levelNumber = kvp.Key;
        ...
        {
            var item = new MenuItem(Level.Name)
            {
                Sensitive = false
            };
            menu.Append(item);
    ...
    {
        Menu menu = [];
        for (var i = 0; i < Weland.plugins.Length; ++i)
        {
            var item = new MenuItem(Weland.plugins.GetName(i));
            var pluginNumber = i;
    ...
    public void OpenFile(string filename)
    {
        ...
        {
            var dialog = new MessageDialog(window1, DialogFlags.DestroyWithParent, MessageType.Error, ButtonsType.Close, e.Message);
            dialog.Run();
            dialog.Destroy();
        }
    }
    ...
    internal void OnOpen(object obj, EventArgs args)
    {
        if (CheckSave())
        {
            var d = new FileChooserDialog("Choose the file to open", window1, FileChooserAction.Open, "Cancel", ResponseType.Cancel, "Open", ResponseType.Accept);
            ...
            {
                Name = "Marathon and Aleph One Maps"
            };
            ...
            if (d.Run() == (int)ResponseType.Accept)
            {
                ...
                var attributesFilename = d.Filename + ".json";
                if (File.Exists(attributesFilename))
                {
                    try
                    {
                        var json = File.ReadAllText(attributesFilename);
                        AllLevelAttributes = JsonSerializer.Deserialize<Dictionary<int, LevelAttributes>>(json, JsonSerializerOptions.Web)!;
    ...
    bool SaveAs()
    {
        ...
        var d = new FileChooserDialog(message, window1, FileChooserAction.Save, "Cancel", ResponseType.Cancel, "Save", ResponseType.Accept);
        ...
        try
        {
            if (d.Run() == (int)ResponseType.Accept)
            {
                ...
                Redraw();
                var levelAndAttributes = Level.Save();
                mapfile.Directory[0] = levelAndAttributes.Wad;
        ...
        {
            var m = new MessageDialog(window1, DialogFlags.DestroyWithParent, MessageType.Error, ButtonsType.Close, "An error occurred while saving.")
            {
                Title = "Save Error",
    ...
    bool Save()
    {
        ...
        else
        {
            ...
            try
            {
                ...
                var attributesFilename = Filename + ".json";
                var json = File.ReadAllText(attributesFilename);
                AllLevelAttributes[LevelIndex] = levelAndAttributes.Attributes;
                File.WriteAllText(attributesFilename, JsonSerializer.Serialize(AllLevelAttributes, JsonSerializerOptions.Web));
    ...
    {
        var d = new FileChooserDialog("Export OBJ as", window1, FileChooserAction.Save, "Cancel", ResponseType.Cancel, "Save", ResponseType.Accept);
        d.SetCurrentFolder(Weland.Settings.GetSetting("LastSave/Folder", Environment.GetFolderPath(Environment.SpecialFolder.Personal)));
        ...
        try
        {
            if (d.Run() == (int)ResponseType.Accept)
            {
                var exporter = new OBJExporter(Level);
        ...
        {
            var m = new MessageDialog(window1, DialogFlags.DestroyWithParent, MessageType.Error, ButtonsType.Close, "An error occured while exporting.")
            {
                Title = "Export error",
                SecondaryText = e.Message
            };
    ...
    internal void OnExportUDB(object obj, EventArgs args)
    {
        var d = new FileChooserDialog("Export script as", window1, FileChooserAction.Save, "Cancel", ResponseType.Cancel, "Save", ResponseType.Accept);
        d.SetCurrentFolder(Weland.Settings.GetSetting("LastSave/Folder", Environment.GetFolderPath(Environment.SpecialFolder.Personal)));
        d.CurrentName = Level.Name + ".js";
        d.DoOverwriteConfirmation = true;
        try
        {
            if (d.Run() == (int)ResponseType.Accept)
    ...
    void BuildHeightPalette(SortedList<short, bool> heights)
    {
        ...
        for (var i = 0; i < paintIndexes.Keys.Count; ++i)
        {
    ...
    void BuildMediaPalette()
    {
        ...
        drawingArea.PaintColors = [];
        paintIndexes.Clear();
        for (var i = -1; i < Level.Medias.Count; ++i)
        {
        ...
        // set up "none"
        var b = new ColorRadioButton(null, "None", new Gdk.Color(127, 127, 127))
        {
            Index = 0
        };
        b.Toggled += OnChangePaintIndex;
        ...
        for (var i = 0; i < paintIndexes.Keys.Count - 1; ++i)
        {
            var c = paintColors[i % paintColors.Count];
    ...
    void BuildAmbientSoundPalette()
    {
        ...
        drawingArea.PaintColors = [];
        paintIndexes.Clear();
        for (var i = -1; i < Level.AmbientSounds.Count; ++i)
        {
            paintIndexes[(short)i] = true;
        }
        editor.PaintIndex = -1;
    ...
    void BuildRandomSoundPalette()
    {
        ...
        drawingArea.PaintColors = [];
        paintIndexes.Clear();
        for (var i = -1; i < Level.RandomSounds.Count; ++i)
        {
            paintIndexes[(short)i] = true;
        }
        editor.PaintIndex = -1;
    ...
    internal void OnGridOriginSet(object o, EventArgs e)
    {
        if (!editor.SetCustomGridOriginToSelected())
        {
            ...
            var dialog = new PointDialog(window1)
            {
                Value = p
            };
            dialog.Run();
    ...
    internal void OnGridRotateSet(object o, EventArgs e)
    {
        ...
        {
            double v;
            var dialog = new DoubleDialog("Set Grid Rotation", window1)
            {
                Value = grid.Rotation
            };
            if (dialog.Run() == (int)ResponseType.Ok && dialog.Valid)
            {
                ...
                else if (v > gridRotationScale.Adjustment.Upper)
                {
                    v = gridRotationScale.Adjustment.Upper;
                }
    ...
    internal void OnGridScaleSet(object o, EventArgs e)
    {
        ...
        {
            double v;
            var dialog = new DoubleDialog("Set Grid Scale", window1)
            {
                Value = grid.Scale
            };
            if (dialog.Run() == (int)ResponseType.Ok && dialog.Valid)
    ...
    {
        var d = new LevelParametersDialog(window1, Level);
        if (d.Run() == (int)ResponseType.Ok)
        {
            editor.Changed = true;
    ...
    {
        var d = new ItemParametersDialog(window1, Level);
        if (d.Run() == (int)ResponseType.Ok)
        {
            editor.Changed = true;
        }
    ...
    {
        var d = new MonsterParametersDialog(window1, Level);
        if (d.Run() == (int)ResponseType.Ok)
        {
            editor.Changed = true;
        }
    ...
    void PaletteNext()
    {
        for (var i = 0; i < paletteButtonbox.Children.Length - 1; ++i)
        {
    ...
    void PalettePrev()
    {
        for (var i = 1; i < paletteButtonbox.Children.Length; ++i)
        {
            if (((ColorRadioButton)paletteButtonbox.Children[i]).Active)
            {
                ((ColorRadioButton)paletteButtonbox.Children[i - 1]).Active = true;
    ...
    protected void OnPaletteAdd(object o, EventArgs e)
    {
        ...
        {
            var dialog = new DoubleDialog("Add Floor Height", window1);
            if (dialog.Run() == (int)ResponseType.Ok && dialog.Valid)
            {
                var height = World.FromDouble(dialog.Value);
                var heights = editor.GetFloorHeights();
                heights[height] = true;
                BuildHeightPalette(heights);
                var b = (ColorRadioButton)paletteButtonbox.Children[paintIndexes.IndexOfKey(height)];
                b.Active = true;
            }
            dialog.Destroy();
        }
        ...
        {
            var dialog = new DoubleDialog("Add Ceiling Height", window1);
            if (dialog.Run() == (int)ResponseType.Ok && dialog.Valid)
            {
                var height = World.FromDouble(dialog.Value);
                var heights = editor.GetCeilingHeights();
                heights[height] = true;
                BuildHeightPalette(heights);
        ...
        else if (editor.Tool == Tool.FloorLight || editor.Tool == Tool.CeilingLight || editor.Tool == Tool.MediaLight)
        {
            ...
            var dialog = new LightParametersDialog(window1, Level, (short)(Level.Lights.Count - 1));
        ...
        else if (editor.Tool == Tool.Media)
        {
            ...
            var dialog = new MediaParametersDialog(window1, Level, (short)(Level.Medias.Count - 1));
            if (dialog.Run() == (int)ResponseType.Ok)
            {
                BuildMediaPalette();
        ...
        else if (editor.Tool == Tool.AmbientSound)
        {
            ...
            var dialog = new AmbientSoundParametersDialog(window1, sound);
            if (dialog.Run() == (int)ResponseType.Ok)
            {
        ...
        {
            var sound = new RandomSound();
            var dialog = new RandomSoundParametersDialog(window1, sound);
            if (dialog.Run() == (int)ResponseType.Ok)
            {
    ...
    void PaletteEdit()
    {
        if (editor.Tool == Tool.FloorHeight)
        {
            var original_height = editor.PaintIndex;
            var dialog = new DoubleDialog("Edit Floor Height", window1)
            {
                Value = World.ToDouble(original_height)
            };
            if (dialog.Run() == (int)ResponseType.Ok && dialog.Valid)
            {
                var height = World.FromDouble(dialog.Value);
        ...
        {
            var original_height = editor.PaintIndex;
            var dialog = new DoubleDialog("Edit Ceiling Height", window1)
            {
                Value = World.ToDouble(original_height)
            };
            if (dialog.Run() == (int)ResponseType.Ok && dialog.Valid)
        ...
        {
            var index = editor.PaintIndex;
            var dialog = new LightParametersDialog(window1, Level, index);
            if (dialog.Run() == (int)ResponseType.Ok)
            {
                editor.Changed = true;
            }
        ...
        else if (editor.Tool == Tool.Media)
        {
            ...
            if (index != -1)
            {
                var dialog = new MediaParametersDialog(window1, Level, index);
                if (dialog.Run() == (int)ResponseType.Ok)
                {
                    editor.Changed = true;
                }
        ...
        else if (editor.Tool == Tool.AmbientSound)
        {
            ...
            if (index != -1)
            {
                var dialog = new AmbientSoundParametersDialog(window1, Level.AmbientSounds[index]);
                if (dialog.Run() == (int)ResponseType.Ok)
                {
                    editor.Changed = true;
                }
        ...
        else if (editor.Tool == Tool.RandomSound)
        {
            ...
            if (index != -1)
            {
                var dialog = new RandomSoundParametersDialog(window1, Level.RandomSounds[index]);
                if (dialog.Run() == (int)ResponseType.Ok)
                {
                    editor.Changed = true;
                }
    ...
    {
        var dialog = new GotoDialog(window1);
        if (dialog.Run() == (int)ResponseType.Ok)
        {
            short index = 0;
            if (short.TryParse(dialog.Number.Text, out index))
            {
                if (dialog.Type.Active == 0)
                {
                    if (index >= 0 && index < Level.Endpoints.Count)
                    {
                        ...
                        var p = Level.Endpoints[selection.Point];
                        Center(p.X, p.Y);
                        UpdateStatusBar();
                        Redraw();
                    }
                ...
                else if (dialog.Type.Active == 2)
                {
                    if (index >= 0 && index < Level.Polygons.Count)
                    {
                        ...
                        var polygon = Level.Polygons[index];
                        var center = Level.PolygonCenter(polygon);
    ...
    {
        var i = Level.FindZeroLengthLine();
        if (i == -1)
        {
            var d = new MessageDialog(window1, DialogFlags.DestroyWithParent, MessageType.Info, ButtonsType.Ok, "No zero length lines found.");
            d.Run();
            d.Destroy();
        }
        else
        {
            var d = new MessageDialog(window1, DialogFlags.DestroyWithParent, MessageType.Warning, ButtonsType.Ok, String.Format("Line {0} has zero length.", i));
            d.Run();
            d.Destroy();
            selection.Clear();
            selectButton.Active = true;
            selection.Line = i;
            var p = Level.Endpoints[Level.Lines[i].EndpointIndexes[0]];
            Center(p.X, p.Y);
            UpdateStatusBar();
            Redraw();
        }
    ...
    void LoadLayer()
    {
        var floorHeight = Weland.Settings.GetSetting(String.Format("Layers/Layer{0}/FloorHeight", layer), -32.0);
        var ceilingHeight = Weland.Settings.GetSetting(String.Format("Layers/Layer{0}/CeilingHeight", layer), 32.0);
    ...
    {
        var d = new PreferencesDialog(window1, drawingArea, editor);
        d.Run();
    ...
    {
        var toEditorName = System.IO.Path.GetTempPath() + Guid.NewGuid().ToString() + ".sceA";
        var fromEditorName = System.IO.Path.GetTempPath() + Guid.NewGuid().ToString() + ".sceA";

        var toEditor = new MapFile();
        Level.AssurePlayerStart();
        Redraw();

        var levelAndAttributes = Level.Save();
        toEditor.Directory[0] = levelAndAttributes.Wad;
        ...
        if (polygonIndex != -1)
        {
            ...
            var scriptName = "Weland Visual Mode Locator";
            ...
            for (var i = 0; i < 66 - scriptName.Length; ++i)
            {
                writer.Write((byte)0);
            }
        ...
        var d = new MessageDialog(window1, DialogFlags.DestroyWithParent | DialogFlags.Modal, MessageType.Other, ButtonsType.None, "Entering Visual Mode...");
        ...
        p.Exited += delegate (object sender, EventArgs e)
        {
            Gtk.Application.Invoke(delegate
            {
                ...
                {
                    var sb = new StringBuilder("Visual Mode Failed!\n");
                    sb.Append("Exception: " + exc.Message + "\n");
                    sb.Append("Arguments: ");
                    sb.Append(String.Join(" ", arguments));

                    var dd = new MessageDialog(null, DialogFlags.DestroyWithParent, MessageType.Error, ButtonsType.Close, sb.ToString());
                    dd.Run();
                    dd.Destroy();
                }
            });
