...
public class Editor
{
    ...
    */
    ...
    {
        var index = Level.GetClosestAnnotation(p);
    ...
    {
        var index = Level.GetClosestPoint(p);
    ...
    {
        var index = Level.GetClosestObject(p);
    ...
    {
        var index = Level.GetClosestLine(p);
    ...
    public void StartLine(short X, short Y)
    {
        ...
        else
        {
            ...
            {
                //split
                var onLine = Level.ClosestPointOnLine(p, Level.Lines[index]);
    ...
    public void ConnectLine(short X, short Y, bool constrainAngle, bool constrainLength)
    {
        ...
        var p = new Point(X, Y);
        var ap = GridAdjust(new Point(X, Y)); //custom grid code
    ...
    void Fill(short X, short Y)
    {
        if (!undoSet)
        {
            LevelAndAttributes prevUndo = null;
            if (undoState != null)
            {
    ...
    {
        var previousSelectedPoly = Selection.Polygon;
        Selection.Clear();
        var p = new Point(X, Y);
        var index = ClosestPoint(p);
        if (index != -1)
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        var polys = Level.GetEnclosingPolygons(p);
                        if (polys.Count > 0)
                        {
                            if (previousSelectedPoly == -1)
                            {
                                Selection.Polygon = polys.First();
                            }
                            else
                            {
                                var selectioIndex = polys.IndexOf(previousSelectedPoly);
                                if (selectioIndex == -1)
                                {
                                    Selection.Polygon = polys.First();
                                }
                                ...
                                {
                                    Selection.Polygon = polys[(selectioIndex + 1) % polys.Count];
                                }
                            }
                        }
                    }
    ...
    void AltSelect(short X, short Y)
    {
        if (Selection.Polygon != -1)
        {
            ...
            {
                var index = Level.GetEnclosingPolygon(new Point(X, Y));
                if (index != -1)
                {
                    SetUndo();
    ...
    void PlaceAnnotation(short X, short Y)
    {
        ...
        {
            var polygon_index = Level.GetEnclosingPolygon(p);
            if (polygon_index != -1)
            {
                SetUndo();
    ...
    void PlaceObject(short X, short Y)
    {
        ...
        else
        {
            ...
            {
                SetUndo();
                var object_index = Level.NewObject(p, polygon_index);
                var o = Level.Objects[object_index];
                if (lastObject != null)
                {
    ...
    void MovePoint(short index, short X, short Y)
    {
        ...
        var lines = Level.EndpointLines[index];
        ...
        var polygons = Level.FindEndpointPolygons(index);
    ...
    public void NudgeSelected(short X, short Y)
    {
        ...
        if (Selection.Point != -1)
        {
            ...
            var newX = (short)(p.X + X);
            var newY = (short)(p.Y + Y);
            /*** custom grid code ***/
            if (Grid.Visible && Grid.Snap)
            {
                if (Grid.UseCustomGrid)
        ...
        else if (Selection.Object != -1)
        {
            ...
            var p = new Point((short)(obj.X + X), (short)(obj.Y + Y));
            var polygon_index = Level.GetEnclosingPolygon(p);
            if (polygon_index != -1)
            {
                obj.X = (short)(obj.X + X);
        ...
        else if (Selection.Polygon != -1)
        {
            ...
            foreach (var annotation in Level.Annotations)
            {
    ...
    void MoveSelected(short X, short Y)
    {
        ...
        else if (Selection.Object != -1)
        {
            ...
            var p = new Point((short)X, (short)Y);
            var polygon_index = Level.GetEnclosingPolygon(p);
            if (polygon_index != -1)
            {
                obj.X = p.X;
    ...
    void SetPolygonType(short X, short Y)
    {
        ...
        if (p != null)
        {
            ...
            var scan = p.Type == PolygonType.Platform || (PolygonType)PaintIndex == PolygonType.Platform;
    ...
    public void Motion(short X, short Y, EditorModifiers mods)
    {
        if (Tool == Tool.Line)
        {
            var constrainAngle = (mods & EditorModifiers.Shift) != 0;
            var constrainLength = (mods & EditorModifiers.Alt) != 0;
    ...
    public void Layer(int layer)
    {
        ...
        {
            SetUndo();
            Level.TogglePolygonLayer(Selection.Polygon, layer);
            Selection.Polygon = -1;
        }
    }
    ...
    public void ToggleLayer()
    {
        ...
        {
            Level.VisibleLayer = 1;
        }
        else if (Level.VisibleLayer < 4)
        {
            Level.VisibleLayer++;
        }
        else
    ...
    public void DeleteSelected()
    {
        ...
        if (Selection.Point != -1)
        {
            ...
            List<short> nextPointCandidates = [];
            foreach (var i in Level.FindEndpointLines(Selection.Point))
            {
            ...
            Line nextLine = null;
            var nextLineEndpointIndex = 0;
            foreach (var point_index in nextPointCandidates)
            {
                foreach (var i in Level.FindEndpointLines(point_index))
                {
    ...
    {
        SortedList<short, bool> list = [];
        foreach (var p in Level.Polygons)
        {
    ...
    public void Recenter()
    {
        ...
        foreach (var p in Level.Endpoints)
        {
        ...
        foreach (var a in Level.Annotations)
        {
        ...
        var marginX = (ushort.MaxValue - (maxX - minX)) / 2;
        var marginY = (ushort.MaxValue - (maxY - minY)) / 2;

        var newMinX = short.MinValue + marginX;
        var newMinY = short.MinValue + marginY;

        var offsetX = (newMinX - minX) / World.One * World.One;
        var offsetY = (newMinY - minY) / World.One * World.One;

        for (var i = 0; i < Level.Endpoints.Count; ++i)
        {
    ...
    public bool SetCustomGridOriginToSelected()
    {
        ...
        {
            var l = Level.Lines[Selection.Line];
            var p1 = Level.Endpoints[l.EndpointIndexes[0]];
            var p2 = Level.Endpoints[l.EndpointIndexes[1]];
    ...
    public bool SetCustomGridScaleToSelected()
    {
        if (Selection.Line != -1)
        {
            ...
            var p1 = Level.Endpoints[l.EndpointIndexes[0]];
            var p2 = Level.Endpoints[l.EndpointIndexes[1]];
            var d = Math.Sqrt(((double)(p2.X - p1.X) * (p2.X - p1.X)) + ((p2.Y - p1.Y) * (p2.Y - p1.Y))) / World.One;
