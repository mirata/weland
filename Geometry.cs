...
public partial class Level
{
    ...
    Dictionary<short, Side> CounterClockwiseOrphanedSides = [];
    HashSet<short> LinesReflectingOrphanedSides = [];
    ...
    public bool FilterPoint(PolygonFilter f, short index)
    {
        ...
        var polygonSet = EndpointPolygons[index];
        ...
        var lineSet = EndpointLines[index];
    ...
    {
        var e0 = Endpoints[line.EndpointIndexes[0]];
        var e1 = Endpoints[line.EndpointIndexes[1]];
    ...
    public int Distance(Point p, Line line)
    {
        ...
        else
        {
            var i = new Point
            {
                X = (short)Math.Round(e0.X + (u * (e1.X - e0.X))),
                Y = (short)Math.Round(e0.Y + (u * (e1.Y - e0.Y)))
            };
            return Distance(p, i);
        }
    }
    ...
    public short GetClosestAnnotation(Point p)
    {
        ...
        {
            var a = Annotations[i];
            var distance = Distance(p, new Point(a.X, a.Y));
            if (distance < min)
            {
    ...
    public short GetClosestPoint(Point p)
    {
        ...
        for (short i = 0; i < Endpoints.Count; ++i)
        {
            ...
            {
                var distance = Distance(p, Endpoints[i]);
                if (distance < min)
                {
                    closest_point = i;
                    min = distance;
    ...
    public short GetClosestObject(Point p)
    {
        ...
        for (short i = 0; i < Objects.Count; ++i)
        {
            ...
            {
                var distance = Distance(p, new Point(Objects[i].X, Objects[i].Y));
                if (distance < min)
                {
                    closest_object = i;
                    min = distance;
    ...
    public short GetClosestLine(Point p)
    {
        ...
        for (short i = 0; i < Lines.Count; ++i)
        {
            ...
            {
                var distance = Distance(p, Lines[i]);
                if (distance < min)
                {
                    closest_line = i;
                    min = distance;
    ...
    public short GetEnclosingPolygon(Point p)
    {
        for (var i = 0; i < Polygons.Count; ++i)
        {
            ...
            {
                var points = GetPointRingFromLineRing(new List<short>(Polygons[i].LineIndexes).GetRange(0, Polygons[i].VertexCount));
    ...
    public List<short> GetEnclosingPolygons(Point p)
    {
        var polygons = new List<short>();
        for (var i = 0; i < Polygons.Count; ++i)
        {
            if (Filter(Polygons[i]))
            {
    ...
    public short SplitLine(short index, short X, short Y)
    {
        ...
        var e0 = line.EndpointIndexes[0];
        var e1 = line.EndpointIndexes[1];
        var p = NewPoint(X, Y);
        ...
        if (line.ClockwisePolygonOwner != -1)
        {
            var polygon = Polygons[line.ClockwisePolygonOwner];

            for (var i = 0; i < Polygon.MaxVertexCount; ++i)
            {
                if (polygon.LineIndexes[i] == index)
                {
                    for (var j = Polygon.MaxVertexCount - 2; j >= i; --j)
                    {
                    ...
                    if (line.ClockwisePolygonSideIndex != -1)
                    {
                        ...
                        s2.Load(reader);
                        s2.LineIndex = l2;
                        var l1_length = (short)Distance(Endpoints[e0], Endpoints[p]);
        ...
        if (line.CounterclockwisePolygonOwner != -1)
        {
            var polygon = Polygons[line.CounterclockwisePolygonOwner];
            for (var i = 0; i < Polygon.MaxVertexCount; ++i)
            {
                if (polygon.LineIndexes[i] == index)
                {
    ...
    {
        SortedDictionary<double, short> candidates = [];
        for (var i = 0; i < Lines.Count; ++i)
        {
            if (FilterLine(Filter, Lines[i]))
            {
                var p0 = Endpoints[Lines[i].EndpointIndexes[0]];
                var p1 = Endpoints[Lines[i].EndpointIndexes[1]];
                ...
                var u = (double)(Y - p0.Y) / (p1.Y - p0.Y);

                var x_intersect = p0.X + (u * (p1.X - p0.X));
    ...
    public List<short> FindEndpointPolygons(short index)
    {
        ...
        for (var i = 0; i < Polygons.Count; ++i)
        {
            for (var j = 0; j < Polygons[i].VertexCount; ++j)
            {
                var line = Lines[Polygons[i].LineIndexes[j]];
    ...
    bool BuildLoop(short target, short prev, short current, int depth, short starter, List<short> list)
    {
        ...
        else if (current == target)
        {
            ...
            var cross = Cross(Diff(Endpoints[prev], Endpoints[current]), Diff(Endpoints[other_index], Endpoints[current]));
            if (cross >= 0.0)
            {
                return true;
            }
        ...
        var neighbors = FindEndpointLines(current);
        ...
        // C# needs a multimap :(
        SortedDictionary<double, List<short>> dotProducts = [];
    ...
    [
        ...
        Lines[lines[0]].EndpointIndexes[0] == Lines[lines[1]].EndpointIndexes[0] ||
        Lines[lines[0]].EndpointIndexes[0] == Lines[lines[1]].EndpointIndexes[1] ? Lines[lines[0]].EndpointIndexes[0] : Lines[lines[0]].EndpointIndexes[1],
    ];
    ...
    double Angle(int X0, int Y0, int X1, int Y1)
    {
        ...
        // in the range [-pi,pi]
        var dtheta = (Math.Atan2(Y0, X0) - Math.Atan2(Y1, X1)) % (Math.PI * 2);
        if (dtheta > Math.PI)
    ...
    bool PointLoopEnclosesPoint(Point p, List<short> points)
    {
        ...
        {
            var p0 = Endpoints[points[i]];
            var p1 = Endpoints[points[(i + 1) % points.Count]];
            var angle = Angle(p0.X - p.X, p0.Y - p.Y, p1.X - p.X, p1.Y - p.Y);
            angle_sum += angle;
        }
    ...
    {
        var candidates = GetFillCandidateLines(X, Y);
        foreach (var kvp in candidates)
        {
            var line = Lines[kvp.Value];
            var p0 = line.EndpointIndexes[0];
            var p1 = line.EndpointIndexes[1];
    ...
    public bool FillPolygon(short X, short Y)
    {
        ...
        {
            VertexCount = (ushort)loop.Count
        };
        loop.CopyTo(polygon.LineIndexes);

        var points = GetPointRingFromLineRing(loop);
        ...
        var index = (short)Polygons.Count;
        Polygons.Add(polygon);
        Polygon adjacent = null;
        for (var i = 0; i < loop.Count; ++i)
        {
            var line = Lines[loop[i]];
            if (line.EndpointIndexes[1] == polygon.EndpointIndexes[i])
            {
                ...
                {
                    var side = ClockwiseOrphanedSides[loop[i]];
            ...
            else
            {
                ...
                {
                    var side = CounterClockwiseOrphanedSides[loop[i]];
    ...
    public void UpdatePolygonConcavity(Polygon polygon)
    {
        ...
        for (var i = 0; i < points.Count; ++i)
        {
            var p0 = Endpoints[points[i]];
            var p1 = Endpoints[points[(i + 1) % points.Count]];
            var p2 = Endpoints[points[(i + 2) % points.Count]];
            var cross = Cross(Diff(p1, p0), Diff(p2, p1));
            if (positive == 0)
            {
                positive = cross;
            }
    ...
    public void DeleteLine(short index)
    {
        ...
        var e0_has_other_lines = false;
        var e1_has_other_lines = false;
        foreach (var other_line in Lines)
        {
    ...
    {
        var adjacent = Polygons[side.PolygonIndex];
        var line = Lines[side.LineIndex];
        ...
        {
            var ceiling_height = adjacent.CeilingHeight;
            var floor_height = adjacent.FloorHeight;
            var opposite_ceiling_height = opposite.CeilingHeight;
            var opposite_floor_height = opposite.FloorHeight;

            if (adjacent.Type == PolygonType.Platform)
            {
                // calculate side based on maxima
                var adjacentPlatform = Platforms[adjacent.Permutation];
            ...
            if (opposite.Type == PolygonType.Platform)
            {
                ...
                var oppositePlatform = Platforms[opposite.Permutation];
            ...
            var platformSafety = (adjacent.Type == PolygonType.Platform || opposite.Type == PolygonType.Platform) && side.Type != SideType.Full;
    ...
    {
        var side_index = (short)Sides.Count;
    ...
    public void Pave()
    {
        var collection = (byte)(Environment + 17);
        const int floor = 6;
        const int ceiling = 7;
    ...
    public short LowestAdjacentFloor(Polygon polygon)
    {
        ...
        foreach (var adjacent in AdjacentPolygons(polygon))
        {
    ...
    public short FindZeroLengthLine()
    {
        ...
        {
            var line = Lines[i];
            var p0 = Endpoints[line.EndpointIndexes[0]];
            var p1 = Endpoints[line.EndpointIndexes[1]];
            if (Distance(p0, p1) == 0)
            {
                return (short)i;
            }
